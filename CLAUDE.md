...
- **`telegram-webhook.js`** is the Telegram-side counterpart to `control.js`/`settings.js` — a remote control channel (`/start`, `/stop`, `/status`, `/strategy`, `/stake`, `/dailygoal`, `/weeklygoal`, `/ask`) for when the dashboard isn't open. Not auto-wired by Netlify; Telegram must be told to POST updates here via `setWebhook` (see the file header). Every command is checked against `TELEGRAM_CHAT_ID` first and silently ignored otherwise — since `control.js`/`settings.js` still have no auth of their own (a known, deliberately deferred gap), this chat-ID check is the *only* thing gating remote control right now. It also handles plain-text (non-command) replies and `approve:`/`deny:` button taps for the LLM supervisor below — everything sits inside the same chat-ID gate, no new entry point bypasses it.
- **`telegram-supervisor.js`** is a second Netlify *scheduled* function (`*/15 * * * *`, independent of `driver.js`'s 1-minute schedule) — the LLM-driven "supervisor" layer. It never trades and never talks to Deriv; it only reads `memory.js` state and, when something notable happened since its last review, calls the Anthropic API to decide whether to send a report, ask a clarifying question, or propose a settings change. See `supervisor-core.js` below for the actual logic (shared with `telegram-webhook.js`'s `/ask` command and free-text-reply handling, so there's one implementation, not two).
...
- **`telegram.js`** — fire-and-forget alerts (bot start, pause/cooldown, daily goal/stop-loss hit, EOD report, trade result) via Telegram's HTTP API directly, using `TELEGRAM_BOT_TOKEN`/`TELEGRAM_CHAT_ID`. No-ops silently if unconfigured. One-way (bot → you); `telegram-webhook.js` is the reverse direction (you → bot), a separate file since it's request-triggered by Telegram rather than called from within a strategy run. These deterministic alerts stay wired into `driver.js` exactly as before and are independent of the supervisor below — safety-relevant notifications (e.g. the kill-switch pause) must fire even if the LLM/Anthropic API is unreachable or unconfigured.
- **`supervisor-core.js`** — the LLM-driven supervisor's actual logic, shared by `telegram-supervisor.js` (the scheduled entry point) and `telegram-webhook.js` (`/ask`, and free-text replies to a pending question), so both callers get one implementation instead of two drifting apart. Gathers a compact snapshot of state/trade history/stats/errors, runs a cheap local `shouldReview` check (no API cost unless something changed — see below), and if warranted calls the Anthropic API (raw `fetch`, no SDK, same pattern as `telegram.js`/`deriv-auth.js`) with `ANTHROPIC_API_KEY`. The model's response resolves to exactly one action: `none` / `report` / `question` / `propose_change`. A `report`/`question` sends a Telegram message via `telegram.js`; a `question` also stores a pending-question record (`memory.js`, global `supervisor_state` key, not per-strategy — see below) that a plain-text reply resolves. A `propose_change` is re-validated through `settings.js`'s existing validators (`validateStakeAmount`, `validateProfitGoal`, `validateStrategy`) *before* it's ever shown to the user, then sent as a Telegram message with Approve/Deny buttons and a pending-approval record — nothing is ever written to `memory.js`'s settings without that human tap, and the tap re-validates against *current* settings again (not the value captured when proposed) in case something changed in between. Only one pending question/approval is allowed at a time (enforced inside `memory.js`'s setters), so a reply is never ambiguous about what it's answering, and a malformed/unparseable model response always fails closed to `none` (`parseSupervisorResponse`) rather than partially applying anything.
...
- `ANTHROPIC_API_KEY` — optional; without it `telegram-supervisor.js` no-ops cleanly on every scheduled tick (mirrors how `telegram.js` no-ops when its own env vars are unset), and `/ask` replies that the supervisor isn't configured.
